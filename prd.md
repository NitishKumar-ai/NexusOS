# PRD: NexusOS Agent Pipeline (Agent Mission Control v2.0)
...
* **OpenClaw**: Control gateway, multi-channel inbox (Discord/WhatsApp), agent routing, and tools platform.
* **Everything-Claude-Code (ECC)**: Agent harness, memory/skills, AgentShield security rules, and lifecycle hooks.
...
TRIGGER LAYER (Phone PWA / CLI / GitHub Webhook)
        |
        v
ORCHESTRATION LAYER (NexusOS Rust Gateway / OpenClaw)
...
AGENT EXECUTION LAYER (Everything-Claude-Code)
  Planner → Architect → Coder → TDD → Reviewer → Security
        |
        v
DELIVERY LAYER (Clawdbot + WebSocket + React PWA + Telegram)
...
Built on **Everything-Claude-Code** agents, each with a specialized role:
...
### Week 1 — Foundation
* Initialize Monorepo: `packages/core-gateway`, `packages/agent-harness`, `dashboard`.
...
* Wire up Planner + Code agents from ECC.
...
root/agent-mission-control
  /packages
    /core-gateway      (formerly openclaw)
    /agent-harness     (formerly everything-claude-code)
    /sdk-bridge        (your adapter)
